...
// ✅ Definir el mapa antes de usarlo
...
// Guardar io para usar en controladores
app.set("socketio", io);
...
io.use((socket, next) => {
  try {
    ...
    console.log(
      "🔑 Token recibido (primeros 20 chars):",
...
io.on("connection", (socket) => {
  ...
  console.log(
    "👥 Total usuarios conectados después de conexión:",
  ...
  socket.on("checkUserOnline", (userId, callback) => {
    ...
    console.log(
      `🔍 Verificando si usuario ${userId} está en línea: ${isOnline}`
    );
  ...
  socket.on("sendMessage", async ({ to, content }) => {
    try {
      ...
      console.log(
        `📤 Enviando mensaje a room: ${to} (¿está en línea? ${isRecipientOnline})`
      );
      ...
      if (isRecipientOnline) {
        ...
        console.log(
          "⚠️ Destinatario no está en línea, mensaje guardado para cuando se conecte"
        );
      }
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (socket.userId) {
      ...
      console.log(
        `🗑️ Usuario removido: ${socket.userId} (estaba conectado: ${wasConnected})`
      );
      console.log(
    ...
    console.log(
      "📊 Total usuarios conectados después de desconexión:",
      io.engine.clientsCount
    );
  });
  ...
  socket.on("updateUserStatus", (status) => {
    if (socket.userId) {
      console.log(
        `🔄 Usuario ${socket.userId} actualizando estado a: ${status}`
      );
...
setInterval(() => {
  ...
  console.log("📊 Estadísticas del servidor:", {
    ...
    timestamp: new Date().toISOString(),
    discrepancy:
      realConnections !== trackedUsers ? "⚠️ DISCREPANCIA DETECTADA" : "✅ OK",
  });
  ...
  if (realConnections !== trackedUsers) {
    ...
    io.sockets.sockets.forEach((socket, id) => {
      console.log(
        `  - Socket ${id}: userId=${socket.userId}, connected=${socket.connected}`
      );
    });
  }
}, 30000);
