      ...
      >
        ...
        <div className="hide-scroll space-y-3">
          {messages?.length === 0 ? (
            ...
            messages
              ?.filter(msg => msg && msg.from && msg.content)
              .map((msg, idx) => {
                const sender = msg.from || 'Desconocido';
                const time = msg.timestamp || 'Sin hora';
                const isCurrentUser = sender === 'Yo';

                return (
                  <div
                    key={idx}
                    className={`flex ${isCurrentUser ? 'justify-end mr-4' : 'justify-start ml-4'}`}
                  >
                    <div
                    ...
                    >
                      ...
                      <span
                        className={`text-xs block mt-1 text-right ${
                          isCurrentUser ? 'text-black dark:text-white' : 'text-black dark:text-white'
                        }`}
                      >
                        {time}
                      </span>
